...
This project covers a basic implementation of the Viola-Jones algorithm through use of openCV and their Haar Cascade files. You can easily utilize the code by downloading the Python Script and any cascades for objects you wish to detect (I currently provide frontal face and side profile face cascades, more are available in the OpenCV Haar Cascades repository). Running the code will allow you to select between using a webcam for live feed detection or uploading an image.

The method itself is simple. By rapidly checking certain haar-like features against the target image, it can determine if certain features - specific light and dark areas- composite a human face. If you use different haar cascades, you can apply the model to detect many other things, but I choose to provide the example with face detection as an easy to understand motivation. For example, we apply the model to a stock image of multiple individuals. 
